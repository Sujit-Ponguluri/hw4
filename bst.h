    ...
    while(root){
        if(key < root->getKey()){
            ...
            if(root->getLeft() && root->getRight()){
                nodeSwap(root,predecessor(root));
            }
...
{
    // TODO
		Node<Key, Value>* root=current->getLeft();
		if(root){
			while(root->getRight()){
					root=root->getRight();
			}
			...
			root=current;
			while(root){
				if(root->getParent() && root->getParent()->getRight()==root){
...
{
    // TODO
		Node<Key, Value>* root=current->getRight();
		if(root){
			while(root->getLeft()){
					root=root->getLeft();
			}
			...
			root=current;
			while(root){
				if(root->getParent() && root->getParent()->getLeft()==root){
					return root->getParent();
				}
				root=root->getParent();
			}
...
template<typename Key, typename Value>
void  BinarySearchTree<Key, Value>::clearTree(Node<Key, Value>* root){
